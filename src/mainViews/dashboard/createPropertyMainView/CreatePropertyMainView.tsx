...
export const CreatePropertyMainView = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data?.success) {
        toast.success("New building project has been successfully saved!");
        router.push("/dashboard/properties");
      }
    } catch (err: any) {
      toast.error(err?.response?.data?.message || "Failed to create property project.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <div>
          ...
          <h2 className="text-xl sm:text-2xl font-black text-[#00062A] tracking-tight flex items-center gap-2">
            ...
            <span>Add New Property / Building</span>
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            Fill in the details below to add a new real estate project listing.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3 pt-4">
          ...
          >
            {submitting ? "Saving Property..." : "Submit & Save Property"}
          </button>
        </div>
      </form>
    </div>
...
export default CreatePropertyMainView;
